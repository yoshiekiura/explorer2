              ...
              db.update_db(settings.coin, function(){
                db.get_stats(settings.coin, function(stats){
                  ...
                  if (mode == 'reindex') {
                    Tx.remove({}, function(err) { 
                      Address.remove({}, function(err2) { 
                        Richlist.update({coin: settings.coin}, {
                          ...
                          var lastblock;  
                          if (body) {
                              lastblock = body.data[0].blockcount;
                              console.log('got last from summary (block: %s)', lastblock);
                            } else {
                              lastblock = stats.count;
                              console.log('got last from stats ');
                            }
                          db.update_tx_db(settings.coin, 1, lastblock, settings.update_timeout, function(){
                    ...
                    db.update_tx_db(settings.coin, stats.last, lastblock, settings.update_timeout, function(){
                      db.update_richlist('received', function(){
                        db.update_richlist('balance', function(){
                          db.get_stats(settings.coin, function(nstats){
