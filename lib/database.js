...
function get_market_data(market, cb) {
  switch(market) {
    ...
    case 'cryptopia':
      ...
      break;
      case 'cryptobridge':
      cryptobridge.get_data(settings.markets.coin, settings.markets.exchange, settings.markets.cryptopia_id, function (err, obj) {
        return cb(err, obj);
      });
      break;
    case 'ccex':
...
module.exports = {
  ...
  get_avg_time: function(count, cb) {
    var sumofblocktime = 0;
    var blockcount = 0;
    Tx.find({'total': {$gt: 0.00000001}}).sort({_id: 'desc'}).limit(count).exec(function(err, txs){
      if (err) {
        return cb(err);
      } else {
        lib.syncLoop(txs.length, function (loop) {
          var i = loop.iteration();
          if((i+1) < txs.length){
            if(txs[i+1].timestamp != txs[i].timestamp) {
              sumofblocktime += (txs[i+1].timestamp - txs[i].timestamp);
              blockcount += 1;
            }
          //console.log('tx diff time: ' + sumofblocktime);
          }
          loop.next();
        }, function(){
          var cavg = (sumofblocktime/blockcount) * -1;
          return cb( { "avg": cavg, "obc": blockcount } );
        });
      }
    });
  },
  ...
  delete_peer: function(addrid, cb) {

      Peers.remove( { "address" : addrid }, function(err, rb) {
        if (err) {
          return cb(null);
        } else {
          if (rb) {
  ...
  /// Summary
  create_summary: function(params, cb) {
    var newSummary = new Summary(params);
  ...
  delete_summary: function(createdAt, cb) {

      Summary.remove( { "createdAt" : createdAt }, function(err, rb) {
        if (err) {
          return cb(null);
        } else {
          if (rb) {
  ...
  // masternodes
  find_masternodes: function(addrs, cb) {
    Masternodes.findOne({addr: addrs}, function(err, masternodes) {
      if (err) {
        return cb(null);
      } else {
        if (masternodes) {
  ...
  create_masternodes: function(params, cb) {
    var newMasternodes = new Masternodes(params);
    newMasternodes.save(function(err) {
      if (err) {
        console.log(err);
        return cb();
      } else {
  ...
  delete_masternodes: function(createdAt, cb) {

      Masternodes.remove( { "createdAt" : createdAt }, function(err, rb) {
        if (err) {
          return cb(null);
        } else {
          if (rb) {
