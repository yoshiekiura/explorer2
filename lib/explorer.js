...
module.exports = {
  ...
  get_masternode_info: function(cb) {
    var uri = base_url + 'masternode?mode=count';
    request({uri: uri, json: true}, function (error, response, body) {
      return cb(body);
    });
  },

  get_masternode_list: function(cb) {
    var uri = base_url + 'masternode?mode=list';
    request({uri: uri, json: true}, function (error, response, body) {
      return cb(body);
    });
  },
  ...
  prepare_vout: function(vout, txid, vin, cb) {
    ...
    module.exports.syncLoop(vout.length, function (loop) {
      ...
      if (vout[i].scriptPubKey.type != 'nonstandard' && vout[i].scriptPubKey.type != 'nulldata') {
        ...
        //console.log('vout:' + i + ':' + arr_vout);
        //console.log(util.inspect(vout[i], false, null))
        if (vout[i].scriptPubKey.hasOwnProperty('addresses')){
        ...
        // no property of addresses, move to next vout
        console.log('vout:' + i + ':' + ' dose not have addresses');
        loop.next();
      }
      } else {
  ...
  get_input_addresses: function(input, vout, cb) {
    ...
    if (input.coinbase) {
      ...
      module.exports.get_rawtransaction(input.txid, function(tx){
        if (tx && tx.hasOwnProperty('vout') != false ) {
